...
/* ----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
/*
Outline: The material point-node pair style will specify how the material points and nodes interact. 
Specifically, it will be about how the material points transfer material information to the nodes to 
create a net force on them, while the nodes transfer no force at all on the material points. 
TO-DO:
* Searchable words: 
        -eliminate: items to eliminate or make unused
        -DEBUG: a section of code whose purpose is for debuggin. Delete once bugs found
        -Complete: section/function has been modified completely
        -Incomplete: further modifications necessary
* New things:
  -> Material Models
        - Add more constitutive relations
        - Add EoS
        - Add fluid EoS and constitutive
        - Add damage models
        - Incorporate temperature
  -> Create grow function for arrays
  -> Create matrix math functions to replace Eigen
  -> 
*/
...
#include <iostream>
#include <Eigen/Eigen>
#include "fix_otm_tlsph_reference_configuration.h"
#include "atom.h"
#include "domain.h"
#include "group.h"
#include "force.h"
...
#include "memory.h"
#include "error.h"
#include "otm_material_models.h"
#include "otm_kernels.h" // eliminate
#include "otm_math.h"
...
/* ---------------------------------------------------------------------- 
  Primarily used to set flags and initial values in the case of pair
  styles.
  ----------------------------------------------------------------------- */

PairOTM::PairOTM(LAMMPS *lmp) :
  Pair(lmp) {

  // failureModel = NULL; // add failure models later

  strengthModel = NULL;
  eos = NULL;

  nmax = 0; // make sure no atom on this proc s.t. initial memory allocation is correct
  R = D = W = NULL;
  CauchyStress = NULL;
  J = NULL;
  ...
  comm_forward = 10; // Communicates stress and mass from ghost mps to nodes
...
PairOTM::~PairOTM() {
  if (allocated) {
    ...
    memory->destroy(hourglass_error);
    memory->destroy(particle_dt);
  }
}

/* ---------------------------------------------------------------------- */

void PairOTM::compute(int eflag, int vflag) {
...
void PairOTM::ComputeForces(int eflag, int vflag) {
...
void PairOTM::settings(int narg, char **arg) {
...
void PairOTM::coeff(int narg, char **arg) {
...
double PairOTM::init_one(int i, int j) {
...
void PairOTM::init_list(int id, class NeighList *ptr) {
  if (id == 0) list = ptr;
}
...
void *PairOTM::extract(const char *str, int &/*i*/) {
//printf("in PairOTM::extract\n");
...
int PairOTM::pack_forward_comm(int n, int *list, double *buf, int /*pbc_flag*/, int * /*pbc*/) {
        int i, j, m;
        tagint *mol = atom->molecule;
        double *damage = atom->damage;
        double *eff_plastic_strain = atom->eff_plastic_strain;
...
//printf("in PairOTM::pack_forward_comm\n");
...
void PairOTM::unpack_forward_comm(int n, int first, double *buf) {
        int i, m, last;
        tagint *mol = atom->molecule;
        double *damage = atom->damage;
        double *eff_plastic_strain = atom->eff_plastic_strain;
...
//printf("in PairOTM::unpack_forward_comm\n");
...
void PairOTM::effective_longitudinal_modulus(const int itype, const double dt, const double d_iso, const double p_rate,
...
/* ----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
void PairOTM::ComputePressure(const int i, const double rho, const double mass_specific_energy, const double vol_specific_energy,
...
/* ----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
void PairOTM::ComputeStressDeviator(const int i, const Matrix3d sigmaInitial_dev, const Matrix3d d_dev, Matrix3d &sigmaFinal_dev,
...
/* ----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
void PairOTM::ComputeDamage(const int i, const Matrix3d strain, const Matrix3d stress, Matrix3d &/*stress_damaged*/) {
        double *eff_plastic_strain = atom->eff_plastic_strain;
        double *eff_plastic_strain_rate = atom->eff_plastic_strain_rate;
        double *radius = atom->radius;
        double *damage = atom->damage;
