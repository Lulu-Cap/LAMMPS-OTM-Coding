...
FixOTMSetVel::FixOTMSetVel(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg) {
  if (narg < 6)
  ...
  if (strstr(arg[3], "v_") == arg[3]) { // This preface allows variable input for fix_vel
    int n = strlen(&arg[3][2]) + 1;
    xstr = new char[n];
    strcpy(xstr, &arg[3][2]); // Assign the str after v_ to xstr
  } else if (strcmp(arg[3], "NULL") == 0) {  
    xstyle = NONE; // No alteration in this dim
  } else {
    xvalue = force->numeric(FLERR, arg[3]);
    xstyle = CONSTANT; // Velocity constant in this dim
  }
...
void FixOTMSetVel::setup(int vflag) {
  if (strstr(update->integrate_style, "verlet"))
    post_force(vflag);
  else
    error->all(FLERR,"Fix otm/setvel does not support RESPA");
}

/* ---------------------------------------------------------------------- */

void FixOTMSetVel::min_setup(int vflag) {
  post_force(vflag);
}

/* ---------------------------------------------------------------------- */

//void FixOTMSetVel::initial_integrate(int vflag) {
void FixOTMSetVel::post_force(int /*vflag*/) {
  double **x = atom->x;
  double **f = atom->f;
  double **v = atom->v;
  // double **vest = atom->vest;//remove
  int *mask = atom->mask;
  ...
  if (varflag == CONSTANT) {
    ...
    // Get variable values this timestep
    ...
    for (int i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        ...
        //DEBUG
        // printf("timestep = %lli\tv[%i] = (%e %e %e)\tf[%i] = (%lf %lf %lf)\n",update->ntimestep,i,v[i][0],v[i][1],v[i][2],i,f[i][0],f[i][1],f[i][2]);
      }
  }
}
...
double FixOTMSetVel::compute_vector(int n) {
...
double FixOTMSetVel::memory_usage() {
  double bytes = 0.0;
  if (varflag == ATOM)
