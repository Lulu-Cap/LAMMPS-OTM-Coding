...
/*
TO-DO:
    * Possibly change identifier to otm/mp_nd/tlsph or similar. This would be useful if I 
      want to have separate pair styles for liquids and solids, which could be useful for 
      ease of implementation in some cases.
    * Remove dependence on Eigen. It is annoying.
*/


#ifdef PAIR_CLASS

PairStyle(otm,PairOTM)

#else

#ifndef LMP_OTM_PAIR_H
#define LMP_OTM_PAIR_H
...
class PairOTM: public Pair {
 public:

  PairOTM(class LAMMPS *);
  virtual ~PairOTM();
 ...
 protected:
  ...
  // Per-atom arrays
  double *J; // Det(F)
  double **R, **D, **W; // rotation matrix, strain rate, and spin tensor
  double **CauchyStress;
  double *hourglass_error;
  double *particle_dt;

  int nmax; // max number of atoms on ths proc
  double hMin; // minimum kernel radius for two particles
  double dtCFL; // max dt based on CFL criterion
  double dtRelative; // relative velocity of two particles divided by speed of sound
  int updateFlag;
  double update_threshold; // updateFlag is set to one if the relative displacement of a pair exceeds update_threshold
  double cut_comm; // cutoff rad w/in which ghost atoms are communicated

  // Enumerate update preferences. Must be in range 5000-6000
  enum {
    UPDATE_NONE = 5000, UPDATE_CONSTANT_THRESHOLD = 5001, UPDATE_PAIRWISE_RATIO = 5002,
  }; 

  // enumerate some quantitities with integer values s.t. they can be used for lookup in an array structure
  // DEBUG: add more as increased # of models are available
  enum {
    LINEAR_DEFGRAD = 0;
  }
