...
void AtomVecOTM::grow(int n) {
  ...
  smd_stress = memory->grow(atom->smd_stress, nmax, NMAT_SYMM, "atom:smd_stress");
...
void AtomVecOTM::copy(int i, int j, int delflag) {
  tag[j] = tag[i];
  type[j] = type[i];
  mask[j] = mask[i];
  image[j] = image[i];
...
int AtomVecOTM::pack_comm_hybrid(int n, int *list, double *buf) {
  int i, j, m;
...
void AtomVecOTM::unpack_comm(int /*n*/, int /*first*/, double * /*buf*/) {
  error->one(FLERR, "atom vec tlsph can only be used with ghost velocities turned on");
}

/* ---------------------------------------------------------------------- */

void AtomVecOTM::unpack_comm_vel(int n, int first, double *buf) {
  int i, m, last;
...
int AtomVecOTM::unpack_comm_hybrid(int n, int first, double *buf) {
  int i, m, last;
...
int AtomVecOTM::pack_reverse(int n, int first, double *buf) {
  int i, m, last;
...
int AtomVecOTM::pack_reverse_hybrid(int n, int first, double *buf) {
  int i, m, last;
...
void AtomVecOTM::unpack_reverse(int n, int *list, double *buf) {
  int i, j, m;
...
int AtomVecOTM::unpack_reverse_hybrid(int n, int *list, double *buf) {
  int i, j, m;
...
int AtomVecOTM::pack_border(int /*n*/, int * /*list*/, double * /*buf*/, int /*pbc_flag*/, int * /*pbc*/) {
  error->one(FLERR, "atom vec tlsph can only be used with ghost velocities turned on");
  return -1;
}
...
int AtomVecOTM::pack_border_vel(int n, int *list, double *buf, int pbc_flag, int *pbc) {
  int i, j, m;
  double dx, dy, dz, dvx, dvy, dvz;

  //printf("AtomVecOTM::pack_border_vel\n");
...
int AtomVecOTM::pack_border_hybrid(int n, int *list, double *buf) {
  int i, j, m;
...
void AtomVecOTM::unpack_border(int /*n*/, int /*first*/, double * /*buf*/) {
  error->one(FLERR, "atom vec tlsph can only be used with ghost velocities turned on");
}

/* ---------------------------------------------------------------------- */

void AtomVecOTM::unpack_border_vel(int n, int first, double *buf) {
  int i, m, last;
...
int AtomVecOTM::unpack_border_hybrid(int n, int first, double *buf) {
  int i, m, last;
...
int AtomVecOTM::pack_exchange(int i, double *buf) {
  int m = 1;

  //printf("in AtomVecOTM::pack_exchange tag %d\n", tag[i]);
...
int AtomVecOTM::unpack_exchange(double *buf) {
...
/* ----------------------------------------------------------------------
 ...
 ------------------------------------------------------------------------- */
int AtomVecOTM::pack_restart(int i, double *buf) {
  int m = 1; // 1
...
int AtomVecOTM::unpack_restart(double *buf) {
  int nlocal = atom->nlocal;
  if (nlocal == nmax) {
    grow(0);
...
int AtomVecOTM::data_atom_hybrid(int /*nlocal*/, char **/*values*/) {
...
void AtomVecOTM::data_vel(int m, char **values) {
...
int AtomVecOTM::data_vel_hybrid(int /*m*/, char **/*values*/) {
  error->one(FLERR, "hybrid atom style functionality not yet implemented for atom style smd");
  return 0;
}
...
void AtomVecOTM::pack_data(double **buf) {
  int nlocal = atom->nlocal;
...
int AtomVecOTM::pack_data_hybrid(int /*i*/, double * /*buf*/) {
  error->one(FLERR, "hybrid atom style functionality not yet implemented for atom style smd");
  return -1;
}
...
void AtomVecOTM::write_data(FILE *fp, int n, double **buf) {
  for (int i = 0; i < n; i++)
    fprintf(fp,
    TAGINT_FORMAT
...
int AtomVecOTM::write_data_hybrid(FILE * /*fp*/, double * /*buf*/) {
  error->one(FLERR, "hybrid atom style functionality not yet implemented for atom style smd");
  return -1;
}
...
void AtomVecOTM::pack_vel(double **buf) {
  int nlocal = atom->nlocal;
  for (int i = 0; i < nlocal; i++) {
    buf[i][0] = ubuf(tag[i]).d;
    buf[i][1] = v[i][0];
...
int AtomVecOTM::pack_vel_hybrid(int /*i*/, double * /*buf*/) {
  error->one(FLERR, "hybrid atom style functionality not yet implemented for atom style smd");
  return 0;
}
...
void AtomVecOTM::write_vel(FILE *fp, int n, double **buf) {
  for (int i = 0; i < n; i++)
...
int AtomVecOTM::write_vel_hybrid(FILE * /*fp*/, double * /*buf*/) {
  error->one(FLERR, "hybrid atom style functionality not yet implemented for atom style smd");
  return 3;
}
...
bigint AtomVecOTM::memory_usage() {
  bigint bytes = 0;
  ...
  if (atom->memcheck("smd_stress"))
    bytes += memory->usage(smd_stress, nmax, NMAT_SYMM);
...
void AtomVecOTM::force_clear(int n, size_t nbytes) {
